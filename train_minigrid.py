...
def main(cfg):
    ...
    def to_env_commands(policy_outputs):
        env_commands = []
        for env_id in range(config.envs.n_threads):
            policy_output = policy_outputs[env_id]
            env_commands.append({agent_id: policy_output[0][agent_id]['action'].item()
                                 for agent_id in policy_output[0]})
        return env_commands
    runner.to_env_commands = to_env_commands
